...
class CombinedAttributesAdder(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, X_transformed):
        ...
        if self.add_bedrooms_per_room:
            ...
            X_original[:, self.bedrooms_ix] = (
                X_transformed[:, self.rooms_ix] * bedrooms_per_room
            )
            X_transformed = np.delete(X_transformed, -1, axis=1)

        population_per_household = X_transformed[:, -1]
        X_original[:, self.population_ix] = (
            X_transformed[:, self.households_ix] * population_per_household
        )
        X_transformed = np.delete(X_transformed, -1, axis=1)

        rooms_per_household = X_transformed[:, -1]
        X_original[:, self.rooms_ix] = (
            X_transformed[:, self.households_ix] * rooms_per_household
        )
        X_transformed = np.delete(X_transformed, -1, axis=1)
